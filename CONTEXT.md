...
**Parameter**:
A named, user-adjustable value declared in a Template, exposed as a Control in the Customize view. Either Annotated (defined via an OpenSCAD Customizer comment, e.g. a range or dropdown) or Inferred (a bare top-level variable with no Customizer comment, given a best-guess Control). A dropdown Control is Annotated either way, even though its options can come from three different comment forms: OpenSCAD's own same-line `// [a,b,c]` bracket syntax; a string Parameter's very next line being nothing but a quoted, comma-separated list (`// "a", "b", "c"`), whose label is just the value itself; or one or more consecutive comment lines each giving a single option its own label (`// "single" = one letter`, `// "all" = A-Z`) — patterns real `.scad` authors use for the same purpose without knowing the stricter syntax, or wanting a friendlier label than the raw value. `parseCustomizer` recognizes all three (checking the labeled-lines form first, then the comma-list); a looser prose comment (`// "a" or "b"`) recognizes none of them and falls back to a plain text Control.
...
**Render**:
Evaluating a Template + Configuration into a Mesh. Always a full evaluation — OpenSCAD Web Management never uses OpenSCAD's fast/approximate "Preview" mode. See [ADR-0001](./docs/adr/0001-always-render-exact-geometry.md). Happens on one of two engines: the client-side one, in a Web Worker through openscad-wasm (kept fast and free by design — see [ADR-0004](./docs/adr/0004-hybrid-client-and-server-rendering.md)); or the server-side one, through native OpenSCAD. Both now run on the Manifold geometry backend (client moved from CGAL in [ADR-0011](./docs/adr/0011-client-side-manifold-backend.md), matching the server-side choice already made in [ADR-0006](./docs/adr/0006-manifold-backend-for-server-side-render.md)) — much faster than CGAL, at the cost of an OpenSCAD-wide caveat about Manifold vs. CGAL disagreeing on some precision-sensitive geometry, which no longer distinguishes the two engines from each other since both use Manifold now. The customer-facing Customize view only ever uses the client-side engine directly, and always auto-renders — even a Template `estimateComplexity` flags likely to be slow (`hasExpensiveLoop`), relying on a render timeout as the safety net instead of an explicit opt-in click (see [ADR-0012](./docs/adr/0012-remove-customer-skip-gate.md), which retired that gate now that ADR-0011 made the client engine fast enough) — though its very first Render for a Template's default Configuration is often served from a precomputed cache instead of computed at all (see [ADR-0010](./docs/adr/0010-cached-default-preview.md)). The Admin Panel keeps its own, separate skip-then-click gate for its live preview during editing (an Admin iterating on raw draft source doesn't want every keystroke risking a hung tab) and can trigger either engine at any time, on demand, via two always-available buttons — see Render on server. See [ADR-0007](./docs/adr/0007-skip-then-server-fallback-for-expensive-preview.md), [ADR-0008](./docs/adr/0008-render-on-server-is-admin-only.md), and [ADR-0009](./docs/adr/0009-admin-save-and-export-rework.md).
